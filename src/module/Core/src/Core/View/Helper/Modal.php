...
class Modal extends AbstractHelper
{
    ...
    public function delete ($text, $href, $class="")
    {
        $id = 'modal_'.uniqid();
        $html = '
<div id="'.$id.'" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p>
            '.$this->getView()->translate('Du bist im Begriff, eine Löschaktion auszuführen. Bei der betreffenden URL handelt es sich um: ').'<b>'.$href.'</b></p><p>'
            .$this->getView()->translate(' Willst du diese Aktion wirklich ausführen?').'        
        </p>
    </div>
